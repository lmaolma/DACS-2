...
<div class="pricing-header px-3 py-3 pt-md-5 pb-md-4 mx-auto text-center">
    ...
    <%
        // URL dạng: JobDetailView.jsp?1
        int jobid = Integer.parseInt(request.getQueryString());
    %>

    <!-- DataSource dùng chung -->
    <sql:setDataSource var="db" driver="com.mysql.cj.jdbc.Driver"
                       url="jdbc:mysql://localhost:3306/jobportal"
                       user="root" password=""/>

    <!-- Lấy đúng job theo jobid -->
    <sql:query var="jobRs" dataSource="${db}">
        SELECT jobid, role, compname, location, visa, comp, jd
        FROM jobs
        WHERE jobid = <%=jobid%>
    </sql:query>

    <c:forEach items="${jobRs.rows}" var="job">
        <ul class="list-group list-group-flush">
            <li class="list-group-item">Job Id : <c:out value="${job.jobid}"/></li>
            <li class="list-group-item">Role : <c:out value="${job.role}"/></li>
            <li class="list-group-item">Company : <c:out value="${job.compname}"/></li>
            <li class="list-group-item">Location : <c:out value="${job.location}"/></li>
            <li class="list-group-item">Visa Sponsorship : <c:out value="${job.visa}"/></li>
            <li class="list-group-item">Compensation : <c:out value="${job.comp}"/></li>
            <li class="list-group-item">Job Description : <c:out value="${job.jd}"/></li>
        </ul>
    </c:forEach>
</div>
...
<div class="pricing-header px-3 py-3 pt-md-5 pb-md-4 mx-auto text-center">
    ...
    <table class="table">
        ...
        <tbody>
        <!--
           Hiện tại bạn chưa có bảng lưu ứng tuyển theo job,
           nên tạm thời truy vấn này sẽ luôn trả về rỗng (không lỗi).
           Sau này tạo bảng applications thì mình sẽ join lại cho đúng.
        -->
        <sql:query var="candRs" dataSource="${db}">
            SELECT cid, name, email, phone
            FROM candidates
            WHERE 1 = 0
        </sql:query>

        <c:forEach items="${candRs.rows}" var="cid">
            <tr>
                <td><c:out value="${cid.cid}"/></td>
                <td><c:out value="${cid.name}"/></td>
                <td><c:out value="${cid.email}"/></td>
                <td><c:out value="${cid.phone}"/></td>
            </tr>
        </c:forEach>
        </tbody>
    </table>
...
<!-- Footer Start -->
<!-- (phần footer giữ nguyên như bạn gửi, mình không đổi gì) -->
<!-- ... toàn bộ footer + js y như file gốc ... -->
